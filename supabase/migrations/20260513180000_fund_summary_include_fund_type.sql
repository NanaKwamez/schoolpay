-- Expose `fund_type` on fund_summary so clients classify rows from the same source as id/name
-- (avoids mismatches when fund ids in the view do not align with a separate `funds` lookup).
