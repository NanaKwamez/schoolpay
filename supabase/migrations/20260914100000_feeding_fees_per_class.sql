-- Sync feeding_today_by_class + school_attendance_today tier amounts with lib/constants getFeedingFeeForClass.
...
SELECT
  ...
  (
    ...
    INNER JOIN students st ON st.id = fl.student_id AND st.class_id = c.id
    CROSS JOIN tz_date z
    WHERE fl.date = z.d AND fl.status = 'covered_weekly'
  ) AS covered_weekly_count,
  (
    SELECT COUNT(DISTINCT fl.student_id)::bigint
    FROM feeding_daily_log fl
    INNER JOIN students st ON st.id = fl.student_id AND st.class_id = c.id
  ...
  (
    SELECT COALESCE(SUM(
      CASE
        WHEN fl.status IN ('paid', 'covered_weekly') THEN COALESCE(
          fl.amount,
          CASE c.name
            WHEN 'Nursery 1' THEN 11::numeric
            WHEN 'Nursery 2' THEN 12::numeric
            WHEN 'KG 1' THEN 13::numeric
            WHEN 'KG 2' THEN 11::numeric
            WHEN 'Basic 1' THEN 12::numeric
            WHEN 'Basic 2' THEN 12::numeric
            WHEN 'Basic 3' THEN 12::numeric
            WHEN 'Basic 4' THEN 12::numeric
            WHEN 'Basic 5' THEN 12::numeric
            WHEN 'Basic 6' THEN 13::numeric
            WHEN 'Basic 7' THEN 13::numeric
            WHEN 'Basic 8' THEN 13::numeric
            WHEN 'Basic 9' THEN 13::numeric
            ELSE 12::numeric
          END
        )
        ELSE 0::numeric
    ...
    FROM feeding_daily_log fl
    INNER JOIN students st ON st.id = fl.student_id AND st.class_id = c.id AND st.is_active = TRUE
    CROSS JOIN tz_date z
    WHERE fl.date = z.d
  ) AS feeding_collected_today
FROM classes c;
...
ft_agg AS (
  SELECT
    COALESCE(SUM(paid_count + credit_count + covered_weekly_count), 0)::bigint AS total_present,
    COALESCE(SUM(absent_count), 0)::bigint AS total_absent,
    COALESCE(SUM(paid_count + credit_count + covered_weekly_count + absent_count), 0)::bigint AS total_marked
