-- Extend feeding_today_by_class: covered_weekly + per-class feeding cash today (Ghana date).
-- Updates school_attendance_today so "present" includes covered_weekly.
    ...
    SELECT COALESCE(SUM(
      CASE
        WHEN fl.status IN ('paid', 'covered_weekly') THEN COALESCE(
          fl.amount,
          CASE
            WHEN c.name IN ('Nursery 1', 'Nursery 2', 'KG 1', 'KG 2') THEN 10::numeric
            WHEN c.name IN ('Basic 1', 'Basic 2', 'Basic 3', 'Basic 4', 'Basic 5') THEN 11::numeric
            ELSE 12::numeric
          END
        )
        ELSE 0::numeric
